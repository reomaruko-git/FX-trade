...
> 最終更新: 2026-02-26（戦略を H&S 3ペア一本化・本番運用版）
...
| ペア | yfinance ticker | 戦略 | バックテスト結果（2年） |
|------|----------------|------|----------------------|
| USD/JPY | `USDJPY=X` | H&S（4時間足） | 70回, 34.3%勝率, +1333 pips |
| EUR/JPY | `EURJPY=X` | H&S（4時間足） | 64回, 28.1%勝率, +1023 pips |
| AUD/JPY | `AUDJPY=X` | H&S（4時間足） | 60回, 55.0%勝率, +2895 pips |

> **3ペア合計**: 194回, 37.6%勝率, **+5251 pips**（500万円 → 約154万円増, 年利約52%）
>
> GBP/JPY は除外（バックテスト: 51回, 17.6%勝率, -22pips で成績不良のため）
...
> **ポジションはSL/TPのみで管理。** 途中でSLを動かさない設計がバックテスト結果と一致する。
...
## バックテスト実績サマリー（2024〜2026年・2年間）

| ペア | 回数 | 勝率 | 累計pips | 損益（20000lot） |
|------|------|------|---------|----------------|
| USD/JPY H&S | 70回 | 34.3% | +1,333 | +266,600円 |
| EUR/JPY H&S | 64回 | 28.1% | +1,023 | +204,600円 |
| AUD/JPY H&S | 60回 | 55.0% | +2,895 | +579,000円 |
| **合計** | **194回** | **37.6%** | **+5,251** | **+1,050,200円** |

> 初期資金500,000円 → 約1,550,200円（年利 **約52%**）
>
